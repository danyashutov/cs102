...
weekdays_russian = {'monday': 'понедельник', 'tuesday': 'вторник', 'wednesday': 'среду', 'thursday': 'четверг', 'friday': 'пятницу', 'saturday': 'субботу', 'sunday': 'воскресенье'}
...
def get_schedule_info(group: str, day: str, week: str = None):
    ...
    if cached_schadule.get(group) is None:
        cached_schadule[group] = {}

    if cached_schadule[group].get(week_key) is None:
        cached_schadule[group][week_key] = {}

    if cached_schadule[group][week_key].get(day) is None:
        #print('Loading web-page with schedule...')
        web_page = get_schedule_page(group, week)
        schedule = parse_schedule_for_day(web_page, day)

        if schedule is None:
            cached_schadule[group][week_key][day] = [[], [], []]
            return None
        else:
            cached_schadule[group][week_key][day] = schedule
            return schedule
    else:
        # print('Loading schedule from cache...')
        schedule = cached_schadule[group][week_key][day]
        if len(schedule[0]) == 0:
            return None
        return schedule
...
def get_schedule(message, week_number: str = None, return_reply=False, consider_week_parity=True):
    ...
    for i in range(len(times_list)):
        resp += f'{times_list[i]} | {lessons_list[i]} | {locations_list[i]}\n'
