...
def group(values: List[str], n: int) -> List[List[str]]:
    ...
    [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    """
    return [[values[rows + i] for i in range(n)] for rows in range(0, len(values), n)]
...
def get_col(grid: List[List[str]], pos: Tuple[int, int]) -> List[str]:
    ...
    ['3', '6', '9']
    """
    return [grid[col_ind][pos[1]] for col_ind in range(len(grid))]
...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    if all(set(get_row(solution, (row_i, 0))) == nums for row_i in range(len(solution))) and \
            all(set(get_row(solution, (0, col_i))) == nums for col_i in range(len(solution[0]))) and \
...
if __name__ == '__main__':
    for fname in ['puzzle1.txt', 'puzzle2.txt', 'puzzle3.txt']:
        ...
        if correct_solution:
            print(f'Solution found! Elapsed time {(time.time() - start_time):.2f} sec')
        else:
            print('WARNING: INCORRECT SOLUTION')
        if not solution:
